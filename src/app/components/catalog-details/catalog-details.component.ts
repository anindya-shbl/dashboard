...
export class CatalogDetailsComponent implements OnInit {
  ...
  pageLoad(): void {
    ...
    this.resetpage();
    window.scrollTo(0, 0);
    this.ctName = this.avtiveRoute.snapshot.paramMap.get('name');
  ...
  getKeyWrdPrd(){
    let url = `product/getKeywordDetails?searchText=${this.ctName}`
    this.CommonService.getCatalogData(url).subscribe((res:any)=>{
      if(res && res.results.length>0){
        this.nameKey = res.results[0].Title
      }
    })
  }
  ...
  getFilterPanel() {
    ...
    let url = `${this.CommonService.searchBaseUrl}filter_panel?reserved_keyword=${this.ctName}&panindia=${this.authService.IsPanIndia}&wh=${this.authService.WHId}&device=1&pincode=${this.authService.PinCode}&ci_csrf_token=&_=${timenow}`;
    this.CommonService.getSearchData(url).subscribe((res: any) => {
      ...
      if (res && res.items && Object.keys(res.items).length > 0) {
        ...
        this.dosageForm = res.items.DosageForm;
        ...
        if (this.dosageForm && this.dosageForm.length > 0) {
          this.dosageForm.forEach((element: any) => {
            element.selected = false;
          });
        }
  ...
  getProductsDetails() {
    let timenow = new Date().getTime();
    let typeFilter = this.selectedType.join('|');
    let priceFilter = this.selectedPrice.join('|');
    let MfgGroup = this.selectedMfg.join('|');
    
    let url = `${this.CommonService.searchBaseUrl}search_reserved_keyword?q=${this.ctName}&panindia=${this.authService.IsPanIndia}&wh=${this.authService.WHId}&pincode=${this.authService.PinCode}&device=1&page=${this.pageNo}&size=${this.pageSize}&category=&selectedDosage=${typeFilter}&MfgGroup=${MfgGroup}&price=${priceFilter}&sortby=${this.selectedSort.sortby}&sortorder=${this.selectedSort.sortorder}`;
  ...
  removeProductWebEngage(data: any) {
   ...
   filterByType(typ: any) {
    this.spinner.show();
    this.pageNo = 1;
    this.pageSize = 50;
    this.totalcount = 0;
    this.isLoading = true;
    if (typ.selected == false) {
      this.selectedType.push(typ.key)
      this.dosageForm.forEach((item: any) => {
        if (item.key == typ.key) {
          item.selected = true;
        }
      });
    } else {
      this.selectedType = this.selectedMfg.filter((ds: any) => ds !== typ.key);
      this.dosageForm.forEach((item: any) => {
        if (item.key == typ.key) {
          item.selected = false;
        }
