...
export class TestDetailsComponent implements OnInit {
  ...
  @ViewChild('allTstDtlMdl') allTstDtlMdl!: ElementRef;
        ...
        if (res.data.Items) {
          ...
          if (this.addedInLabCart.length > 0) {
            this.addedInLabCart.forEach((item: any) => {
              if ((res.data.Items.ServiceId == item.ServiceId) && (item.PkgServiceId == undefined || item.PkgServiceId == null || item.PkgServiceId == '')) {
                res.data.Items.IsAdded = true;
              }
            });
          };
          this.labTestDetails.push(res.data.Items);
          // this.getFaqs(this.labTestDetails[0].ServiceId)
  ...
  addLabTest(productObj: any) {
    ...
    if (canAdd == true) {
      ...
      if (this.alertMsg != '') {
        this.allTstDtlMdl.nativeElement.click();
      }
    }
  }

  updateLabTestList(tmp: any) {
    this.labTestDetails = this.labTestDetails.map((obj: any) => {
  ...
  updatebySearch() {
    this.dbService.getAll('LabTests').subscribe((res: any) => {
      // console.log('all records', res);
      this.addedInLabCart = res;
      this.labTestDetails.forEach((obj: any) => obj.IsAdded = false);

      if (this.addedInLabCart.length > 0) {
        this.labTestDetails.forEach((elm: any) => {
          this.addedInLabCart.forEach((item: any) => {
