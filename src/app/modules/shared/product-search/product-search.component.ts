...
export class ProductSearchComponent implements OnInit {
  ...
  count() {
    this.dbService.count('cartItems').subscribe((res: any) => {
      ...
      this.cartCount = res;
      this.allCount = this.cartCount + this.labCount;     
    });
  ...
  onSearch() {
    ...
    let searchUrl = `${this.CommonService.searchBaseUrl}product_list_v2?q=${this.searchText}&page=${this.pageNo}&size=${this.size}&ptype='P'&wh=${werehouseId}&panindia=${panindia}&pincode=${pincode}&token=${token}&strict_match=1&mtype='P'&includeGiftable=0&include_discontinued=0&timestamp=${timenow}&format=2`;
  ...
  gotocartMob(type: any){ 
    if(type == 'customercart'){
      window.location.href = `${this.authService.baseurl}customercart`;
    }
    else if(type == 'labcart'){
      window.location.href = `${this.authService.baseurl}customerlabcart`;
    }
  }

  showMultiCart(){
    this.multiCart = !this.multiCart;
  }
}
